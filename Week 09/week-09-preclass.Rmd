...
stock_selectors <- html_table(tables[[2]])
stock_selectors
```
...
```{r}
stock_selectors$`3 Month % Change`
```
...
```{r}
html_nodes(tables[[1]], ".wsod_symbol") %>%
...
```{r}
html_nodes(webpage, ".HeadlineList") %>%
  html_nodes("[href]") %>%
  html_text()
```
...
```{r}
html_nodes(webpage, ".wsod_StockTimestamp") %>%
  html_attrs()
```
...
```{r}
tables[[3]] %>%
  html_nodes("[class^=bars]") %>%
  #remove comments if you want to remove "bars pct" from each value
  html_attr("class") #%>%
  #gsub(pattern="bars pct",replacement="")
```
